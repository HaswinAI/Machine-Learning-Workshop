...
# Global variables for saving alphabets
detected_alphabets = []
alphabet_save_file = "detected_alphabets.txt"
...
def main():
    ...
    # Initialize variables for alphabet saving
    last_detected_alphabet = ""
    last_detection_time = 0
    detection_cooldown = 1.0  # Minimum seconds between detections of the same alphabet
    alphabet_history = deque(maxlen=5)  # Track recent detections for stability
    ...
    while True:
        ...
        if key == 27:  # ESC
            break
        elif key == ord('s'):  # S key to save current alphabet sequence
            save_alphabets_to_file(detected_alphabets)
        elif key == ord('c'):  # C key to clear detected alphabets
            detected_alphabets.clear()
            last_detected_alphabet = ""
        ...
        if mode == 2:
            ...
            current_detected_alphabets = []  # Track alphabets detected in this frame
            ...
            if results.multi_hand_landmarks is not None:
                for hand_landmarks, handedness in zip(
                    ...
                    # Get the detected alphabet
                    detected_alphabet = ""
                    if hand_sign_id < len(keypoint_classifier_labels):
                        detected_alphabet = keypoint_classifier_labels[hand_sign_id]
                    
                    # Add to history for stability checking
                    if detected_alphabet:
                        alphabet_history.append(detected_alphabet)
                        current_detected_alphabets.append(detected_alphabet)
                        
                        # Check if we have consistent detections
                        if len(alphabet_history) >= 3:
                            # Check if the last 3 detections are the same
                            if (all(a == detected_alphabet for a in list(alphabet_history)[-3:]) and
                                detected_alphabet != last_detected_alphabet):
                                # Add to detected alphabets list
                                detected_alphabets.append(detected_alphabet)
                                last_detected_alphabet = detected_alphabet
                                last_detection_time = cv.getTickCount() / cv.getTickFrequency()
            ...
            debug_image = draw_alphabet_display(debug_image, detected_alphabets)
...
def draw_alphabet_display(image, detected_alphabets):
    """Draw the detected alphabets on the screen"""
    # Draw background rectangle
    height, width = image.shape[:2]
    cv.rectangle(image, (10, height - 100), (width - 10, height - 10), (0, 0, 0), -1)
    cv.rectangle(image, (10, height - 100), (width - 10, height - 10), (255, 255, 255), 2)
    
    # Draw title
    cv.putText(image, "Detected Alphabets:", (20, height - 80), 
               cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv.LINE_AA)
    
    # Draw alphabets
    if detected_alphabets:
        alphabet_text = " ".join(detected_alphabets)
        # Wrap text if too long
        if len(alphabet_text) > 50:
            alphabet_text = alphabet_text[:47] + "..."
        cv.putText(image, alphabet_text, (20, height - 50), 
                   cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
    else:
        cv.putText(image, "None", (20, height - 50), 
                   cv.FONT_HERSHEY_SIMPLEX, 0.8, (100, 100, 100), 2, cv.LINE_AA)
    
    # Draw instructions
    cv.putText(image, "Press 'S' to save, 'C' to clear", (20, height - 20), 
               cv.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1, cv.LINE_AA)
    
    return image

def save_alphabets_to_file(alphabets):
    """Save detected alphabets to a text file"""
    if not alphabets:
        print("No alphabets to save")
        return
        
    try:
        # Create a timestamped filename
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"detected_alphabets_{timestamp}.txt"
        
        with open(filename, 'w') as f:
            f.write("American Sign Language Detection Results\n")
            f.write("=" * 40 + "\n")
            f.write(f"Detection Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write("Detected Alphabets:\n")
            f.write("-" * 20 + "\n")
            for i, alphabet in enumerate(alphabets, 1):
                f.write(f"{i}. {alphabet}\n")
            f.write(f"\nTotal Alphabets Detected: {len(alphabets)}\n")
        
        print(f"Alphabets saved to {filename}")
        
        # Also save to the default file
        with open(alphabet_save_file, 'w') as f:
            f.write(" ".join(alphabets) + "\n")
            
        print(f"Alphabets also saved to {alphabet_save_file}")
    except Exception as e:
        print(f"Error saving alphabets: {e}")
